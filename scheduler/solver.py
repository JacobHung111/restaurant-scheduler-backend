...
def generate_schedule_with_ortools(
    ...
    logger.info(
        f"  - Added: Minimize Demand Shortage (weight: {WEIGHT_DEMAND_SHORTAGE})"
    )
    ...
    logger.info(
        f"  - Added: Minimize Min Hour Shortage (weight: {WEIGHT_MIN_HOUR_SHORTAGE})"
    )
    ...
    # Handle other statuses
        ...
        logger.warning(
            f"[OR-Tools] Solver stopped with status: {solver.StatusName(status)}"
        )
        warnings.append(
